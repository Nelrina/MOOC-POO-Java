...
class Tirelire {
    ...
    public void afficher() {
        if(montant > 0) System.out.println("Vous avez : " + montant + " euros dans votre tirelire.");
        else System.out.println("Vous etes sans le sou.");
    }
    
    public void secouer() {
        if(montant > 0) System.out.println("Bing bing");
    }
    ...
    public void puiser(double m) {
        if(m > 0 && m <= montant) montant -= m;
        else if(m > montant) montant = 0;
    }
    
    public double calculerSolde(double b) {
        if (b <= 0) return montant;
        else return montant - b;
    }
}
